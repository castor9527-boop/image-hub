...
function assertAllowedUpstreamUrl(url) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    throw new Error('上游地址无效');
  }
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error('上游地址协议无效');
  }

  const env = getRuntimeEnv();
  if (IS_DEV && String(env.NOVA_ALLOW_ANY_UPSTREAM || 'true').toLowerCase() !== 'false') {
    return parsed.toString().replace(/\/$/, '');
  }
  const configuredHosts = String(env.NOVA_ALLOWED_API_HOSTS || '')
    .split(',')
  ...
  if (allowedHosts.size === 0 || !allowedHosts.has(parsed.hostname.toLowerCase())) {
    throw createHttpError(403, 'UPSTREAM_NOT_ALLOWED', '该上游地址未被服务端允许');
  }
  return parsed.toString().replace(/\/$/, '');
}
...
const ASSET_DIR = process.env.GGOO_ASSET_DIR || path.join(__dirname, 'ggoo-assets');
const taskRefImages = new Map();
const taskAccessTokens = new Map(); // taskId -> hashed capability token
...
function hashTaskAccessToken(token) {
  return createHash('sha256').update(String(token || '')).digest('hex');
}

function createTaskAccessToken() {
  return randomUUID().replaceAll('-', '') + randomUUID().replaceAll('-', '');
}

function hasTaskAccess(taskId, token) {
  const expected = taskAccessTokens.get(taskId);
  if (!expected || typeof token !== 'string' || token.length < 32) return false;
  const received = Buffer.from(hashTaskAccessToken(token), 'utf8');
  const stored = Buffer.from(expected, 'utf8');
  return received.length === stored.length && timingSafeEqual(received, stored);
}

function requireTaskAccess(taskId, token) {
  if (!hasTaskAccess(taskId, token)) {
    throw createHttpError(401, 'TASK_ACCESS_REQUIRED', '任务访问凭证无效或已过期');
  }
}
...
function ensureAssetDir() {
  try {
    ...
    console.error(`[asset-storage] 无法创建素材目录: ${ASSET_DIR}`, error);
    process.exit(1);
  }
}
...
function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_assets_owner_updated ON assets(owner_id, updated_at);
  `);
...
function getAssetOwnerId(req) {
  const value = String(req.headers['x-ggoo-workspace-id'] || '').trim();
  if (!/^[a-zA-Z0-9._:-]{16,160}$/.test(value)) {
    throw createHttpError(401, 'ASSET_OWNER_REQUIRED', '素材库访问凭证无效');
  }
  return hashTaskAccessToken(value);
}

function validateAssetId(id) {
  if (typeof id !== 'string' || !/^[a-zA-Z0-9._:-]{1,180}$/.test(id)) {
    throw createHttpError(400, 'INVALID_ASSET_ID', '素材 ID 无效');
  }
  return id;
}

function getAssetBlobPath(assetId, mimeType = 'image/png') {
  const ext = getImageExtension(mimeType);
  return path.join(ASSET_DIR, `${assetId}.${ext}`);
}
...
function parseAssetDataUrl(dataUrl) {
  ...
  if (!buffer.length || buffer.length > 8 * 1024 * 1024) {
    throw createHttpError(413, 'ASSET_TOO_LARGE', '素材文件过大');
  }
...
function serializeAssetRow(row) {
  if (!row) return null;
  let metadata = {};
  try { metadata = JSON.parse(row.metadata_json); } catch { /* invalid legacy row */ }
  return {
    ...metadata,
    id: row.id,
    kind: row.kind,
    createdAt: Date.parse(row.created_at) || metadata.createdAt,
    updatedAt: Date.parse(row.updated_at) || metadata.updatedAt,
    remoteBlobUrl: row.blob_file ? `/api/ggoo/assets/${encodeURIComponent(row.id)}/blob` : undefined,
  };
}

async function handleAssetApi(req, res, pathname) {
  const ownerId = getAssetOwnerId(req);
  const match = pathname.match(/^\/api\/ggoo\/assets(?:\/([^/]+)(?:\/(blob))?)?$/);
  if (!match) return false;
  const assetId = match[1] ? validateAssetId(decodeURIComponent(match[1])) : null;
  const isBlob = match[2] === 'blob';

  if (req.method === 'GET' && !assetId) {
    const rows = db.prepare('SELECT * FROM assets WHERE owner_id = ? ORDER BY updated_at DESC').all(ownerId);
    sendJson(res, 200, { assets: rows.map(serializeAssetRow) });
    return true;
  }

  if (req.method === 'GET' && assetId && isBlob) {
    const row = db.prepare('SELECT * FROM assets WHERE owner_id = ? AND id = ?').get(ownerId, assetId);
    if (!row?.blob_file || !fs.existsSync(row.blob_file)) {
      sendJson(res, 404, { error: '素材文件不存在' });
      return true;
    }
    const stat = fs.statSync(row.blob_file);
    pipeFileToResponse(res, row.blob_file, 200, {
      'Content-Type': getContentType(row.blob_file),
  ...
  if (req.method === 'POST' && !assetId) {
    ...
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) {
      throw createHttpError(400, 'INVALID_ASSET', '素材信息无效');
    }
    const id = validateAssetId(body.id);
    const kind = body.kind === 'text' ? 'text' : body.kind === 'image' ? 'image' : null;
    if (!kind) throw createHttpError(400, 'INVALID_ASSET_KIND', '素材类型无效');
    const now = new Date().toISOString();
    const parsedBlob = kind === 'image' ? parseAssetDataUrl(body.dataUrl) : null;
    const blobFile = parsedBlob ? getAssetBlobPath(id, parsedBlob.mimeType) : null;
    if (parsedBlob) fs.writeFileSync(blobFile, parsedBlob.buffer);
    db.prepare(`
      INSERT INTO assets (id, owner_id, kind, metadata_json, blob_file, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(owner_id, id) DO UPDATE SET kind = excluded.kind, metadata_json = excluded.metadata_json,
      blob_file = COALESCE(excluded.blob_file, assets.blob_file), updated_at = excluded.updated_at
    `).run(id, ownerId, kind, JSON.stringify(metadata), blobFile, now, now);
    sendJson(res, 201, { asset: serializeAssetRow(db.prepare('SELECT * FROM assets WHERE owner_id = ? AND id = ?').get(ownerId, id)) });
    return true;
  }

  if (req.method === 'PATCH' && assetId && !isBlob) {
    const body = await readJsonBody(req);
    const current = db.prepare('SELECT * FROM assets WHERE owner_id = ? AND id = ?').get(ownerId, assetId);
    if (!current) { sendJson(res, 404, { error: '素材不存在' }); return true; }
    const metadata = { ...JSON.parse(current.metadata_json), ...(body?.metadata || {}) };
    const now = new Date().toISOString();
    db.prepare('UPDATE assets SET metadata_json = ?, updated_at = ? WHERE owner_id = ? AND id = ?')
      .run(JSON.stringify(metadata), now, ownerId, assetId);
    sendJson(res, 200, { asset: serializeAssetRow(db.prepare('SELECT * FROM assets WHERE owner_id = ? AND id = ?').get(ownerId, assetId)) });
    return true;
  }

  if (req.method === 'DELETE' && assetId && !isBlob) {
    const current = db.prepare('SELECT * FROM assets WHERE owner_id = ? AND id = ?').get(ownerId, assetId);
    if (!current) { sendJson(res, 404, { error: '素材不存在' }); return true; }
    if (current.blob_file) fs.rmSync(current.blob_file, { force: true });
    db.prepare('DELETE FROM assets WHERE owner_id = ? AND id = ?').run(ownerId, assetId);
...
function createTask(body, req) {
  ...
  const accessToken = createTaskAccessToken();
  const now = new Date().toISOString();
  const requestForDb = {
    mode: body.mode,
    source: 'nova',
    ...
    sceneMetadata: body.sceneMetadata,
  };
  ...
  taskAccessTokens.set(taskId, hashTaskAccessToken(accessToken));
  // 递增 pending 计数
...
async function requestGptImage(apiKey, request, resolvedSize, options = {}) {
  const baseUrl = assertAllowedUpstreamUrl(options.baseUrl || resolveNovaApiBaseUrl());
...
function deleteTask(taskId) {
  ...
  tx();
  taskAccessTokens.delete(taskId);
  cleanupTaskRuntimeState(taskId);
  broadcastQueueStatus();
}
...
function handleSubscribeTasks(ws, taskIds, taskTokens = {}) {
  ...
  for (const id of taskIds.slice(0, WS_MAX_TASK_IDS_PER_MESSAGE)) {
    ...
    const token = taskTokens && typeof taskTokens === 'object' ? taskTokens[id] : undefined;
    if (!hasTaskAccess(id, token)) {
      safeSendJson(ws, { type: 'error', code: 'TASK_ACCESS_REQUIRED', message: '任务访问凭证无效或已过期' });
      continue;
    }
...
function handleClientMessage(ws, raw) {
  ...
  switch (msg.type) {
    case 'subscribeTasks':
      handleSubscribeTasks(ws, msg.taskIds, msg.taskTokens);
      break;
...
async function handleApi(req, res, pathname) {
  try {
    ...
    if (apiPathname.startsWith('/api/ggoo/assets')) {
      return await handleAssetApi(req, res, apiPathname);
    }
    ...
    if (req.method === 'POST' && apiPathname === '/api/nova/proxy/text') {
      try {
        ...
        const normalizedBaseUrl = assertAllowedUpstreamUrl(normalizeProtocolBaseUrl(protocol, baseUrl));
        let targetUrl;
    ...
    if (req.method === 'POST' && apiPathname === '/api/nova/tasks') {
      ...
      sendJson(res, 202, { taskId: task.taskId }, {
        'X-Task-Access-Token': task.accessToken,
      });
      return true;
    }
    ...
    if (req.method === 'GET' && !action) {
      requireTaskAccess(taskId, req.headers['x-task-access-token']);
...
const startServer = () => {
  ...
  const httpServer = http.createServer(async (req, res) => {
    const parsedUrl = parseNodeUrl(req.url || '/', true);
    if (parsedUrl.pathname?.startsWith('/api/nova/') || parsedUrl.pathname?.startsWith('/api/ggoo/')) {

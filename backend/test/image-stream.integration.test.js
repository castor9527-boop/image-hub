...
test('retries an unsupported partial-images request without stream parameters', async t => {
  ...
  const child = spawn(process.execPath, ['server.js'], {
    ...
    env: {
      ...
      NOVA_ALLOWED_API_HOSTS: '127.0.0.1',
    },
  ...
  const taskAccessToken = createResponse.headers.get('x-task-access-token');
  assert.ok(taskAccessToken);

  const task = await waitFor(async () => {
    const response = await fetch(`${backendUrl}/api/nova/tasks/${taskId}`, {
      headers: { 'X-Task-Access-Token': taskAccessToken },
    });

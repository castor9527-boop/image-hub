import { getNovaTask, getNovaQueueStatus, getNovaTaskAccessToken, type NovaTaskResponse, type NovaQueueStatus } from '@/lib/ccode-task-client';
...
class NovaTaskSocket {
  ...
  subscribeTask(taskId: string, handler: TaskUpdateHandler): () => void {
    ...
    if (this.fallbackActive) {
      ...
      this.send({ type: 'subscribeTasks', taskIds: [taskId], taskTokens: this.getTaskTokens([taskId]) });
    }
  ...
  private getTaskTokens(taskIds: string[]): Record<string, string> {
    return Object.fromEntries(
      taskIds
        .map(taskId => [taskId, getNovaTaskAccessToken(taskId)] as const)
  ...
  private connect(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      if (taskIds.length > 0) {
        this.send({ type: 'subscribeTasks', taskIds, taskTokens: this.getTaskTokens(taskIds) });
      }

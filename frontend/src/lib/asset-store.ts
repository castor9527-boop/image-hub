...
import { deleteGgooAsset, migrateGgooImageAsset, syncGgooAsset, syncGgooAssetMetadata } from '@/lib/ggoo-asset-sync';
...
export async function addImageAsset(input: AddImageAssetInput): Promise<ImageAsset> {
  ...
  if (sameSourceAsset) {
    ...
    void syncGgooAssetMetadata(updated).catch(() => undefined);
    return updated;
  }
  ...
  void migrateGgooImageAsset(asset, sourceBlob).catch(() => undefined);
  return asset;
}
...
export async function addTextAsset(input: AddTextAssetInput): Promise<TextAsset> {
  ...
  void syncGgooAsset(asset).catch(() => undefined);
  return asset;
}
...
export async function listAssets(kind?: AssetKind): Promise<AssetItem[]> {
  ...
  const assets = await getAllFromStore<AssetItem>(db, ASSETS_STORE);
  db.close();
  if (!migrationStarted && assets.length > 0) {
    migrationStarted = true;
    void Promise.all(assets.map(async asset => {
      if (isImageAsset(asset)) {
        await migrateGgooImageAsset(asset, await getAssetBlob(asset.id));
      } else {
        await syncGgooAsset(asset);
      }
    })).catch(() => undefined);
  }
  return assets
...
export async function touchAsset(assetId: string): Promise<void> {
  ...
  db.close();
  if (!asset) return;
  const updated = { ...asset, lastUsedAt: now(), updatedAt: now() };
  await putAssetAndBlob(updated, null);
  void syncGgooAssetMetadata(updated).catch(() => undefined);
}
...
export async function deleteAsset(assetId: string): Promise<void> {
  ...
  void deleteGgooAsset(assetId).catch(() => undefined);
}

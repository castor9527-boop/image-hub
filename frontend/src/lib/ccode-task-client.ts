...
import type { ScenePromptMetadata } from '@/lib/scene-prompts';
...
export interface CreateNovaTaskInput {
  ...
  sceneMetadata?: ScenePromptMetadata;
}
...
const TASK_TOKEN_PREFIX = 'ggoo-task-token:';
const taskAccessTokens = new Map<string, string>();

function getTaskTokenStorageKey(taskId: string): string {
  return `${TASK_TOKEN_PREFIX}${taskId}`;
}

export function getNovaTaskAccessToken(taskId: string): string | undefined {
  const memoryToken = taskAccessTokens.get(taskId);
  if (memoryToken) return memoryToken;
  if (typeof window === 'undefined') return undefined;
  try {
    return window.sessionStorage.getItem(getTaskTokenStorageKey(taskId)) || undefined;
  } catch {
    return undefined;
  }
}

function saveNovaTaskAccessToken(taskId: string, token: string): void {
  taskAccessTokens.set(taskId, token);
  if (typeof window === 'undefined') return;
  try {
    window.sessionStorage.setItem(getTaskTokenStorageKey(taskId), token);
  } catch {}
}
...
export async function createNovaTask(input: CreateNovaTaskInput): Promise<string> {
  ...
  const accessToken = response.headers.get('X-Task-Access-Token');
  if (accessToken) saveNovaTaskAccessToken(data.taskId, accessToken);
  return data.taskId;
}
...
export async function getNovaTask(taskId: string): Promise<NovaTaskResponse> {
  const response = await fetchWithTimeout(`/api/nova/tasks/${encodeURIComponent(taskId)}`, {
    method: 'GET',
    cache: 'no-store',
    headers: getNovaTaskAccessToken(taskId) ? { 'X-Task-Access-Token': getNovaTaskAccessToken(taskId)! } : undefined,

...
import { SceneGenerationWorkspace } from '@/components/SceneGenerationWorkspace';
...
export function WorkspaceShell() {
  ...
  const [activeTab, setActiveTab] = useState<'image-generation' | 'scene-generation' | 'agent' | 'canvas' | 'assets' | 'reverse-prompt' | 'gif' | 'prompt-gallery'>('scene-generation');
  const [scenePromptDraft, setScenePromptDraft] = useState('');
  const [sceneMetadataDraft, setSceneMetadataDraft] = useState<ScenePromptMetadata | undefined>();
  const [sceneReferenceDraft, setSceneReferenceDraft] = useState<RefImageData[]>([]);
  const [sceneAspectRatioDraft, setSceneAspectRatioDraft] = useState<SceneAspectRatio>('1:1');
  ...
  const generationInitialData = useMemo(() => (
    workspace.retryData
      ...
      : scenePromptDraft ? { prompt: scenePromptDraft, sceneMetadata: sceneMetadataDraft, refImages: sceneReferenceDraft, aspectRatio: sceneAspectRatioDraft } : undefined
  ), [sceneAspectRatioDraft, sceneMetadataDraft, scenePromptDraft, sceneReferenceDraft, workspace.retryData]);
      ...
      <div className={cn(
        ...
        <div className={cn(
          ...
          <WorkspaceHeader
            ...
            showDeveloperSettings={SHOW_DEVELOPER_SETTINGS}
          ...
          >
              ...
              {wideMode && (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate text-base font-semibold tracking-tight leading-tight">GGOO 图片创作</h2>
                    <p className="truncate text-[11px] text-muted-foreground leading-tight">场景化图片创作工作台</p>
                  </div>
                </button>
              )}
            ...
            <div className={cn(
              ...
              <TabsContent value="scene-generation" keepMounted className={cn(wideMode ? 'xl:min-h-0 xl:flex xl:flex-1 xl:flex-col' : 'space-y-6')}>
                <SceneGenerationWorkspace
                  wideMode={wideMode}
                  disabled={!workspace.hasApiKey}
                  onConfigureApiKey={() => setSettingsOpen(true)}
                  onOpenFreeCreation={(prompt, metadata, refImages, options) => {
                    if (prompt) setScenePromptDraft(prompt);
                    if (metadata) setSceneMetadataDraft(metadata);
                    if (refImages) setSceneReferenceDraft(refImages);
                    if (options) setSceneAspectRatioDraft(options.aspectRatio);
                    setActiveTab('image-generation');
                  }}
                />
              </TabsContent>

...
import { addImageAsset } from '@/lib/asset-store';
...
export function useWorkspaceJobs() {
  ...
  const completeJob = useCallback(async (jobId: string, job: StoredJob) => {
    ...
    if (job.status === 'completed' && job.images?.length) {
      await Promise.all(job.images.map(async (image, index) => {
        try {
          ...
          await addImageAsset({
            blob,
            name: `GGOO 生成结果-${job.id.slice(0, 8)}-${index + 1}`,
            tags: ['生成结果', ...(job.sceneMetadata ? ['场景生成'] : [])],
            sourceKind: job.mode === 'image-to-image' ? 'image-to-image' : 'text-to-image',
            sourceLabel: job.sceneMetadata ? 'GGOO 场景生成' : 'GGOO 图片创作',
            sourceRef: job.id,
            prompt: job.prompt,
          });
        } catch {}
